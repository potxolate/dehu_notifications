...
class DehuController(http.Controller):
    @http.route('/dehu/notification/update', type='json', auth='none', methods=['POST'])
            ...
            for notif_data in notifications:
                ...
                if fecha:
                    try:
                        ...
                        fecha_odoo = fields.Datetime.to_string(date_parser.parse(fecha))
                    except Exception:
                        fecha_odoo = False
                else:
                    fecha_odoo = False
                ...
                if notification:
                    ...
                    Notification.create({
                        ...
                        'available_date': fecha_odoo,
                        'issuer_entity': notif_data.get('organismoEmisor', {}).get('nombreOrganismo'),
                        'holder_nif': notif_data.get('titular', {}).get('nifTitular'),
                        'holder_name': notif_data.get('titular', {}).get('nombreTitular'),
                        'status': 'pending'
                    })
            
            return {'status': 'success', 'message': 'Notificaciones procesadas'}
        except Exception as e:
            _logger.error("Error processing DEHú notifications: %s", str(e))
